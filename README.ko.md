...
| `ENABLE_SEO`       | 검색엔진 색인을 허용하려면 `true`로 설정 (기본: 비활성)        | No   | —             |
| `SITE_URL`         | 공개 도메인. canonical URL·sitemap·링크 미리보기에 사용        | No   | Vercel 도메인 |
...
공개 모드라고 해서 검색엔진에 노출되어야 하는 것은 아닙니다 — 크롤러 기본 설정은 [검색엔진 색인](#-검색엔진-색인)을 참고하세요.
...
**색인은 기본적으로 비활성화되어 있습니다.** 모든 배포는 `noindex, nofollow`와 크롤링을 차단하는 `robots.txt`를 제공하므로, 직접 호스팅하거나 포크한 인스턴스는 명시적으로 켜지 않는 한 검색 결과에 나타나지 않습니다.

특히 공개 모드에서 중요합니다. 이 설정이 없으면 인스턴스가 제공 중인 레포지토리 이름과 그 안의 코드까지 검색엔진에 색인될 수 있습니다.

`ENABLE_SEO=true`는 공개 데모나 랜딩 사이트에만 설정하세요. 활성화하더라도 레포지토리·커밋·풀 리퀘스트 화면은 계속 차단되며, 언어별 랜딩 페이지만 색인됩니다. canonical URL이 `localhost`로 폴백되지 않도록 `SITE_URL`도 함께 설정하세요.

이 설정은 링크 미리보기에 영향을 주지 않습니다. Open Graph 태그는 항상 출력되므로, 슬랙이나 디스코드에 링크를 공유하면 어느 쪽이든 제목과 설명이 표시됩니다.
